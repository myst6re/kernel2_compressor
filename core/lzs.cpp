...
qint32 LZS::match_length =
    0; // of longest match. These are set by the InsertNode() procedure.
qint32 LZS::match_position = 0;
qint32 LZS::lson[4097]; // left & right children & parents -- These constitute
                        // binary search trees.
qint32 LZS::rson[4353];
qint32 LZS::dad[4097];
unsigned char LZS::text_buf[4113]; // ring buffer of size 4096, with extra 17
...
const QByteArray &LZS::decompress(const char *data, int fileSize, int max)
{
	...
	if (quint64(sizeAlloc) > 2000 * quint64(fileSize)) {
		qWarning() << "LZS::decompress impossible ratio case" << sizeAlloc
		           << 2000 * quint64(fileSize);
		result.clear();
		return result;
	}
	...
	forever
	{
		if (((premOctet >>= 1) & 256) == 0) {
			premOctet = *fileData++ | 0xff00; // On récupère le premier octet
			                                  // puis on avance d'un octet
		}
		...
		if (premOctet & 1) {
			result[curResult] = text_buf[curBuff] =
			    *fileData++; // On récupère l'octet (qui n'est pas compressé) et
			                 // on le sauvegarde dans la chaine finale (result)
			                 // et dans le buffer. Et bien sûr on fait avancer
			                 // les curseurs d'un octet.
			curBuff = (curBuff + 1) & 4095; // Le curseur du buffer doit
			                                // toujours être entre 0 et 4095
			++curResult;
		} else {
			//			quint16 twoBytes = *((const quint16 *)fileData);
			//			adresse = (twoBytes & 0x00FF) | ((twoBytes & 0xF000) >>
			//4);
			...
			adresse = *fileData++;
			length = *fileData++;
			adresse |= (length & 0xF0) << 4; // on récupère l'adresse dans les
			                                 // deux octets (qui sont
			                                 // "compressés")
			length = (length & 0xF) + 2 + adresse;

			for (i = adresse; i <= length; ++i) {
				result[curResult] = text_buf[curBuff] =
				    text_buf[i & 4095]; // On va chercher l'octet (qui est
				                        // décompressé) dans le buffer à
				                        // l'adresse indiquée puis on le
				                        // sauvegarde dans la chaine finale et
				                        // le buffer.
...
{
	/* Inserts string of length 18, text_buf[r..r+18-1], into one of the trees
	(text_buf[r]'th tree) and returns the longest-match position and length via
	the global variables match_position and match_length.
	If match_length = 18, then removes the old node in favor of the new one,
...
const QByteArray &LZS::compress(const char *data, int sizeData)
{
	...
	// initialize trees
	/* For i = 0 to 4095, rson[i] and lson[i] will be the right and left
	children of node i. These nodes need not be initialized.
	Also, dad[i] is the parent of node i. These are initialized to 4096 which
	stands for 'not used.'
	For i = 0 to 255, rson[4097 + i] is the root of the tree for strings that
	begin with character i. These are initialized to 4096. Note there are 256
	trees. */
	...
	code_buf[0] = 0; // code_buf[1..16] saves eight units of code, and
	                 ...
	                 // the unit is an unencoded letter (1 byte), "0" a
	                 // position-and-length pair (2 bytes). Thus, eight units
	...
	//	for(i=s ; i<r ; ++i)
	//		text_buf[i] = '\x0';//Clear the buffer with  any character that will
	//appear often.
	memset(text_buf, 0, r);
	...
	for (i = 1; i <= 18; ++i)
		InsertNode(r - i); // Insert the 18 strings, each of which begins with
		                   // one or more 'space' characters.  Note the order in
		                   // which these strings are inserted.  This way,
		                   // degenerate trees will be less likely to occur.

	InsertNode(r); // Finally, insert the whole string just read.  The global
	               // variables match_length and match_position are set.

	do {
		if (match_length > len)
			match_length = len; // match_length may be spuriously long near the
			                    // end of text.
		...
		if (match_length <= 2) {
			...
			code_buf[code_buf_ptr++] =
			    (unsigned char)(((match_position >> 4) & 0xf0)
			                    | (match_length - (2 + 1))); // Send position
			                                                 // and length pair.
			                                                 // Note
			                                                 // match_length >
			                                                 // 2.
		...
		for (i = 0; i < last_match_length && data < dataEnd; ++i) {
			...
			if (s < 17)
				text_buf[s + 4096] = c; // If the position is near the end of
