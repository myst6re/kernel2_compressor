...
public:
	...
	static const QByteArray &decompress(const char *data, int fileSize,
	                                    int max);
	...
	static const QByteArray &decompressAllWithHeader(const char *data,
	                                                 int size);
...
private:
	...
	static qint32 match_length; // of longest match. These are set by the
	                            // InsertNode() procedure.
	static qint32 match_position;
	static qint32 lson[4097]; // left & right children & parents -- These
	                          // constitute binary search trees.
	static qint32 rson[4353];
	static qint32 dad[4097];
	static unsigned char text_buf[4113]; // ring buffer of size 4096, with extra
	                                     // 17 bytes to facilitate string
	                                     // comparison
